...
@pytest.mark.parametrize("jit_compile", [True, False], ids=["jit", "eager"])
def test_stochastic_coulomb_forces_at_backbone(
    backbone_positions_single_residue,
    simple_positions,
    simple_charges,
    jit_compile,
):
    """Test that stochastic backbone forces work and have correct shape."""
    fn = compute_stochastic_coulomb_forces_at_backbone
    if jit_compile:
        fn = jax.jit(fn)
    # Create backbone charges (5 atoms per residue)
    backbone_charges = jnp.ones((1, 5)) * 0.5  # 1 residue, 5 atoms
